...
import { Upload, Download, FileJson, FileText, FolderDown, AlertCircle, Check } from 'lucide-react'
...
export function ImportExportPanel({ onClose }: ImportExportPanelProps) {
  const { collections, activeCollectionId, _userId: userId } = usePromptBooksStore()
      ...
      try {
        ...
        if (file.name.endsWith('.json')) {
          ...
          if (data.prompts && Array.isArray(data.prompts)) {
            await service.importFromJson(client, userId!, data, targetCollectionId || undefined)
        ...
        importResult.failed++
        importResult.errors.push(`${file.name}: ${err instanceof Error ? err.message : 'Unknown error'}`)
      }
    }
  ...
  const handleExportAll = useCallback(async () => {
    ...
    try {
      const client = createClient()
      const allData: Record<string, unknown> = { exportedAt: new Date().toISOString(), collections: [] }

      for (const collection of collections) {
        const data = await service.exportCollectionAsJson(client, collection.id)
        ;(allData.collections as unknown[]).push({ ...data, collectionName: collection.name })
      }
  ...
  return (
    <div className="space-y-6">
      {/* Import Section */}
      <section>
        <h3 className="text-xs font-display text-text-primary mb-3 uppercase tracking-wider">Import</h3>

        <div className="glass-subtle rounded-xl p-4 space-y-3">
          {/* Target collection */}
          <div>
            <label className="text-[10px] font-sans text-text-muted mb-1 block">Import into collection</label>
            <select
          ...
          {importing && (
            <p className="text-xs font-sans text-brand-accent animate-pulse">Importing...</p>
          )}

          {result && (
            <div className={cn(
              'rounded-lg p-3 text-xs font-sans',
              result.failed > 0 ? 'bg-error/10 text-error' : 'bg-success/10 text-success',
            )}>
              <div className="flex items-center gap-1.5">
                {result.failed > 0 ? <AlertCircle className="w-3.5 h-3.5" /> : <Check className="w-3.5 h-3.5" />}
                <span>{result.success} imported, {result.failed} failed</span>
              </div>
      ...
      {/* Export Section */}
      <section>
        <h3 className="text-xs font-display text-text-primary mb-3 uppercase tracking-wider">Export</h3>
        ...
        <div className="glass-subtle rounded-xl p-4 space-y-2">
          ...
          {collections.length > 0 && (
            <div className="pt-2 border-t border-white/[0.04] space-y-1">
              <p className="text-[10px] font-sans text-text-muted mb-1">Export single collection:</p>
              {collections.map((c) => (
                <button
                  key={c.id}
                ...
                >
                  ...
                  {c.name}
                  <span className="text-[10px] text-text-muted/50 ml-auto">{c.promptCount} prompts</span>
                </button>
              ))}
            </div>
          )}
