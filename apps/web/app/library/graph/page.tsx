  ...
  return (
    <main className="mx-auto max-w-7xl px-6 pb-24 pt-12">
      ...
      <nav className="mb-12 flex items-center gap-4 border-b border-white/10 pb-4">
        <Link
          href="/library"
          className="rounded-full border border-white/10 px-4 py-2 text-sm text-text-muted hover:border-crystal/50 hover:text-crystal transition-colors"
        >
          Browse Library
        </Link>
        <Link
        ...
        <Link
          href="/library/graph"
          className="rounded-full bg-crystal px-4 py-2 text-sm font-semibold text-cosmic-deep"
        >
          Relationship Graph
        </Link>
      </nav>
      ...
      <div className="mb-12 grid gap-4 md:grid-cols-4">
        <div className="rounded-xl border border-white/10 bg-cosmic-surface p-6">
          <p className="text-3xl font-bold text-crystal">{graph.nodes.length}</p>
          <p className="text-sm text-text-muted">Total Texts</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-cosmic-surface p-6">
          <p className="text-3xl font-bold text-brand-gold">{collections.length}</p>
          <p className="text-sm text-text-muted">Collections</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-cosmic-surface p-6">
          <p className="text-3xl font-bold text-fire">{graph.edges.length}</p>
      ...
      {/* Legend */}
      <div className="mt-12 rounded-xl border border-white/10 bg-cosmic-surface p-6">
        <h2 className="mb-4 text-xs font-semibold uppercase tracking-[0.35em] text-crystal">
          Connection Types
        </h2>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          <div className="flex items-center gap-3">
            <span className="h-3 w-8 rounded-full bg-crystal" />
            <span className="text-sm text-text-secondary">Related</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="h-3 w-8 rounded-full bg-brand-gold" />
            <span className="text-sm text-text-secondary">Prerequisite</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="h-3 w-8 rounded-full bg-fire" />
