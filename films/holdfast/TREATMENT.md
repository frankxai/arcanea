# HOLDFAST — beat sheet

> **Runtime 4:14 total** — 4:10 of picture (beats 1–14) plus a 0:04 end card.
> Fourteen beats. Turn at 2:47. **Speech-free budget ≥ 50s (20%).**
>
> The card is counted, not excluded. Beat 14 is not shortened to absorb it: the
> final rise is the film's release and it needs its twelve seconds. Both figures
> sit well inside the festival's 3:00 floor and ≤5:00 recommendation, so the
> honest number costs nothing. Measured at stage 8, not estimated.
> Camera law: never above standing eye-height until beat 14.
> Dialogue is **indicative only** — lines are written at stage 4 against the
> Spoken Law:
> [`frankxai/claude-skills-library`](https://github.com/frankxai/claude-skills-library)
> `packs/film-excellence/LANGUAGE.md`. That pack also carries `film-design.md`,
> `film-taste.md`, and the `film-release-gate` skill; craft lives there, canon
> lives here. Merged to `main` 2026-08-20
> ([#23](https://github.com/frankxai/claude-skills-library/pull/23)) — the links
> resolve.
>
> **Two different things are called silence in this film and they must not be
> confused.** *Speech-free* means nobody is talking — the 174 Hz bed, wind, and
> stone continue underneath, and this is the 20% budget. *Total silence* means
> no signal at all, and it happens exactly once, at beat 14, where it is the
> climax. Everywhere else, a signal-free track would read as a dropout.

---

| # | TC | Beat | Camera | Sound |
|---|---|---|---|---|
| 1 | 0:00–0:22 | **Hands.** Close, low. A young woman's hands setting a stone into a wall — checking the bed, seating it, thumbing away spall. Steady. Competent. Doing this well. No face. | Knee height, locked, no move | 174 Hz bed in from frame one. Stone-on-stone. Wind. No music. |
| 2 | 0:22–0:40 | Wider — the yard. Rain-wet grey-green stone, a low wall half built, weather coming. She works alone. Behind her, out of focus, warm gold windows of the Academy up the hill. She never looks at them. | Standing height, static | Bed. Distant bell from the hill, unremarked. |
| 3 | 0:40–1:02 | **Vesk arrives.** Coat, instrument, glasses in a pocket. *"Kess Andal."* Not a question. She doesn't stop working. He asks if she knows why he's here. She tells him the east wall sweats after rain. | Eye-height, he enters frame; she stays low | First dialogue. Bed continues under. |
| 4 | 1:02–1:22 | **The measuring.** He sets the brass instrument on the wall she built. Crystal, needle. *"Hold still."* The needle reads nothing. He notes it. Eleventh time on the sheet. Her face for the first time — flat, waiting, the shadows under the eyes. | Instrument in close-up at wall height | Instrument clicks. Bed. |
| 5 | 1:22–1:38 | **First wrongness.** Dust on the sill beside the instrument falls *upward* — two frames, unmissable, uncommented. Neither character sees it. Her hand, out of frame, has tightened. | Static, sill in foreground | Bed. One frame of the sub thickening, then normal. |
| 6 | 1:38–2:00 | He runs the number. Rank 0, eleven terms. He explains the file closes at end of term, in the passive voice. She asks what happens to the wall. He doesn't understand the question. Neither does the audience. | Two-shot, both at standing height, wide space between them | Bed. Wind up. |
| 7 | 2:00–2:16 | **Speech-free beat 1 (16s).** She goes back to work. He watches, writing. She sets one stone. Her arms are shaking and she seats it anyway. He notices the shake and writes something different. | Slow, low push on the shaking hands — the film's only push | Bed and stone only. No dialogue. |
| 8 | 2:16–2:31 | He asks, offhand, when she last slept. She says the stairs are long. He starts to write it as evasion — stops. Looks at the yard properly for the first time. Something about where she has chosen to build. | His eyeline, then his POV across the yard | Bed. Writing stops. |
| 9 | 2:31–2:47 | **The switch.** He puts the pen down. Active voice: *"Show me your hand."* She doesn't. He waits. She gives him the left one — the two fingers that won't straighten. He turns it over. Stone dust. Callus in a pattern that isn't from masonry. | Close, both hands in frame, low | Bed. Everything else drops out. |
| 10 | **2:47** | **THE TURN.** He looks past her at the wall she has been building — and sees it is not a wall. It is buttressing. Reveal: set into the ground behind it, low, half-covered, a door of black Kaelith Stone. Around its seam, amber-white light. The first saturated color in the film. He looks at her hand, at the door, at her hand. *He understands before we do.* Then we do. | The reveal is a slow rack-focus past her shoulder — camera does not move | Bed **thickens** for the first time. Nothing else. |
| 11 | 2:47–3:12 | He asks how long. She says *since I was six.* First direct answer in the film — and the audience realizes every earlier answer was also direct, about the room, because the room was the only safe subject. He asks why she never said. She says nobody asked about the wall. | Static two-shot, low, door in frame between them | Bed. |
| 12 | 3:12–3:40 | He puts his hand on the door. Instantly his arms shake — the tremor we saw in her at beat 7, arriving in a fit adult in under four seconds. He takes it off, breathing hard. Wipes his eye with his wrist. Looks at her. Eleven terms. *"Sit down,"* he says. She says she can't — **and this is where her "hold" lands** (see `CHARACTERS.md` § Word Ledger conflicts: his institutional *"Hold still"* at beat 4, hers literal here). The exact line is set at stage 4, but the collision is load-bearing and must survive drafting. He says he knows. He sits down instead, on the ground, beside the door, and puts his hand back. | Both at ground level — the lowest the camera goes. It has worked below eye-height all film without descending steadily; this is its floor, not the end of a slope | Bed. His breathing. Her breathing. |
| 13 | 3:40–3:58 | **Speech-free beat 2 (18s).** Two people holding a door. The seam-light on both faces. At 3:44, in the light: a shape behind the seam, too large to resolve, eleven frames. Neither reacts. It is not a jump scare; it is scale. Kess's shoulders drop a fraction — the first time in the film her body has changed. | Static, low, holding | Bed only. Then, at 3:56, the bed begins to *thin*. |
| 14 | 3:58–4:10 | **The break.** The 174 Hz stops. Total silence — the first the film has ever had, and the audience feels the absence of something they never knew was there. And the camera rises. Past the wall. Past the yard. Higher than it has been allowed to go all film, and it keeps going: doors set into the ground across the entire valley, hundreds of them, seams glowing. Some of them are dark. Some of them are **open**. | **The rule breaks.** The only vertical move in the film. | **Silence.** Nothing. Not one sound cue. |
| — | 4:10 | Title card: **HOLDFAST**. Four seconds. Black. No music, no sting. | — | Silence. |

---

## Structural notes

**Speech-free budget:** beats **1** (22s), **2** (18s), **5** (16s), **7** (16s),
**10** (0s), **13** (18s), **14** (12s) = **102s of 250s picture — 40.8%**,
against a 20% floor. Measured at stage 8, not estimated.

Seven beats, and they are exactly the complement of the seven that carry
dialogue (3, 4, 6, 8, 9, 11, 12) — **derive the set that way, never by hand.**
**Beat 10 is the turn: an instant, not a span**, so it contributes 0s while
still belonging to the set.

The denominator is **picture (250s), not total (254s)**, and the four-second
card is excluded from numerator and denominator alike. Excluding it from one
and not the other is the picture/total conflation this document warns about two
sections down.

Beat 2 counts — the distant bell is sound, not dialogue — and so does beat 1,
which means **the film's first spoken word arrives at 0:40**. Forty seconds of a
woman building a wall before anyone says anything is the opening argument: she
is competent, she is alone, and the film is in no hurry.

**Why the turn is a rack focus, not a camera move.** The film's grammar has been
static and low for 167 seconds. A dolly or crane on the biggest moment would
break the rule early and spend the ending. Changing *focus* rather than
*position* keeps the rule intact while making the reveal feel like a mind
changing rather than a camera finding. It is also the shot most AI video models
handle worst, which means almost nobody will attempt it — and doing it well is a
craft signature a jury reads instantly.

**Why he sits down.** The film could end at the turn with him certifying her.
That would be a story about an institution correcting itself, which is
bureaucratic and cold. Him sitting down is a story about a person choosing to
take some of the weight — the one action available to him, chosen at cost, in
silence. It converts the film's subject from *recognition* to *company*, which
is the version people will remember.

**Why the last shot has no sound.** Every other entry will land the ending on a
swell. The absence of the bed *is* the emotional event — the audience's body has
been carrying 174 Hz for four minutes without knowing it, and the relief is
physical. Adding music would cover the only truly novel thing in the film.

---

## The three deliberate imperfections (Doctrine Law 5)

| Shot | What goes wrong |
|---|---|
| Beat 2 | Rain hits the lens and stays for six seconds. Nobody wipes it. |
| Beat 7 | Focus misses on the shaking hands and hunts back — one full breath out of focus. |
| Beat 12 | A gust takes dust across frame and obscures both faces for 1.5 seconds at the emotional peak of the beat. We do not cut around it. |

## The three hand shots (Doctrine Law 5)

| Shot | Task |
|---|---|
| Beat 1 | Setting and seating a stone — full sequence, close, no cuts. |
| Beat 9 | Vesk turning her left hand over, reading the callus. |
| Beat 12 | His hand going onto the Kaelith Stone, and the tremor arriving. |

---

Built on SIP.
